...
(function() {
	...
	L.Routing.ORS = L.Class.extend({
			...
			for (i = 0; i < response.routes.length; i++) {
				...
				alts.push({
					...
					summary: {
						...
						totalTime: path.summary.duration / 1000,
					},
		...
		_convertInstructions: function(instructions) {
			...
			for (i = 0; i < instructions.length; i++) {
				...
				for (j = 0; j < instr.steps.length; j++) {
					step = instr.steps[j];
					result.push({
		...
		_mapWaypointIndices: function(waypoints, instructions, coordinates) {
			...
			for (i = 0; i < instructions.length; i++) {
				instr = instructions[i];
				for (j = 0; j < instr.steps.length; j ++){
					if (instr.steps[j].type === 10) { // VIA_REACHED
						idx = instr.steps[j].way_points[0];
						wpIndices.push(idx);
						wps.push({
	...
	L.Routing.ors = function(options) {
		return new L.Routing.ORS(options);
	};

	module.exports = L.Routing.ORS;
})();
